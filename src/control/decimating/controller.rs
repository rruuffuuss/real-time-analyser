use super::super::control_core::ControlCore;
...
impl DecimatingController {
    ...
    pub fn run(&mut self) {
        let (fresh_tx, fresh_rx): (Sender<Box<[f32]>>, Receiver<Box<[f32]>>) = mpsc::channel();
        let (stale_tx, stale_rx): (Sender<Box<[f32]>>, Receiver<Box<[f32]>>) = mpsc::channel();
        ...
        let transfer_slice = vec![0_f32; self.control_core.transform_size].into_boxed_slice();
        ...
        } = &mut self.control_core.merger_settings
        {
            {
                *bars_per_octave = spectrum_chunk_size;
                *starting_note_offset = -1 * (spectrum_chunk_size as isize);
            }
        }

        let mut transformer = Transformer::new(
            transform_size,
            self.control_core.merger_settings.build(
                transform_size,
                spectrum_chunk_size,
                self.control_core.sample_rate,
            ),
        );

        thread::spawn(move || captor::run(transform_size as usize, fresh_tx, stale_rx));
        stale_tx.send(transfer_slice).unwrap();
        ...
        for recieved in fresh_rx {
            ...
            while cur_chunk < decimations - 1 {
                ...
                let (target, source) = sample_buffer.split_at_mut(chunk_size * cur_chunk + tap_num);

                //copy 'partially used' samples from the end of the chunk to the start
                target[chunk_size * cur_chunk..]
                    .copy_from_slice(&source[transform_size - tap_num..transform_size]);
